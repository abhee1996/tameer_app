...
async function getChat(conversation_id, userId) {
  const url =
    config.server +
    'api/message/getChat?conversationId=' +
    conversation_id +
    '&userId=' +
    userId +
    '&name=sender';
...
function ChatScreen({route, navigation}) {
  ...
  const {conversationId, receiverId, userId, title, name, number} =
    route.params;
    ...
    return (
      <Bubble
        ...
        wrapperStyle={{
          ...
          right: {
            backgroundColor: showDelete ? colors.secondary : '#69d7fa',
            marginRight: 10,
            borderTopLeftRadius: 7,
            borderTopRightRadius: 7,
            borderBottomLeftRadius: 7,
  ...
  const getFurtherMessages = () => {
    getChat(conversationId, userId).then(data => {
      setMessages(
        data.map(item => {
